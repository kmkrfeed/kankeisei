...
async function step_intro(){
  ...
  queueLinesFromBlock(
    ...
    "辺りを見回すと、{{player}}はエントランスのような場所の中心に立っていたことに気づく。<br>" +
    "灯りはついていない。<br>" +
...
async function step_q1_frequency(){
  ...
  queueLinesFromBlock(
    (STATE.timeline==="current")
      ? "記録係：{{player}}と《${STATE.target}》の関係は現在進行形であると認識しました。<br>記録係：以降は現在形で質問・記録します。<br>記録係：それでは、次の部屋に移動します。<br>記録係：私が誘導しますので順路に従ってお進みください。<br>壁面では先程までと変わらず色鮮やかな写真が流れている。<br>次の部屋へと先導する記録係の後ろで、{{player}}は歩みを進めるのだった。<br>記録係：——ご連絡です。第二の部屋に到着いたします。"
      : "記録係：{{player}}と《${STATE.target}》の関係は過去に属するものであると認識しました。<br>記録係：以降は過去形で質問・記録します。<br>記録係：それでは、次の部屋に移動します。<br>記録係：私が誘導しますので順路に従ってお進みください。<br>壁面を見ると、先程までとは違い、セピア色の古い写真が流れている。<br>次の部屋へと先導する記録係の後ろで、{{player}}は歩みを進めるのだった。<br>記録係：——ご連絡です。第二の部屋に到着いたします。"
  );
  await waitQueueEmpty();
...
async function step_q2_valence(){
  ...
  queueLinesFromBlock(
    (STATE.valence==="neg")
      ? "記録係：{{player}}が《${STATE.target}》に抱く感情に負の傾向を確認しました。<br>記録係：以降の展示ではこの傾向を前提に質問・記録します。<br>記録係：それでは、次の部屋に移動します。<br>記録係：私が誘導しますので順路に従ってお進みください。<br>辺りを見れば、先程までと違い、青く凍てついたフラスコが三割増で増えている。<br>今この時も中身は変化しているようだ。<br>次の部屋へと先導する記録係の後ろで、{{player}}は歩みを進めるのだった。<br>記録係：——ご連絡です。第三の部屋に到着いたします。"
      : "記録係：{{player}}が《${STATE.target}》に抱く感情に好意的な傾向を全体もしくは一部に確認しました。<br>記録係：以降の展示ではこの傾向を前提に質問・記録します。<br>記録係：それでは、次の部屋に移動します。<br>記録係：私が誘導しますので順路に従ってお進みください。<br>辺りを見れば、先程までと違い、赤いフラスコと透明なフラスコの数が三割増で増えている。<br>今この時も中身は変化しているようだ。<br>次の部屋へと先導する記録係の後ろで、{{player}}は歩みを進めるのだった。<br>記録係：——ご連絡です。第三の部屋に到着いたします。"
  );
  await waitQueueEmpty();
...
async function step_q3_heart_share(){
  ...
  queueLinesFromBlock(
    ...
    "その傍らには、{{player}}の背丈ほどもある丸い球状のガラスが台座の上に置かれていた。<br>" +
...
async function step_q9_trust(){
  ...
  queueLinesFromBlock(
    ...
    "その中の一つ、{{player}}の目線に近い場所に置かれた鍵式の金庫の前で、記録係は足を止めた。<br>" +
    "上を見れば、ガラス張りの天井から丸い月が覗いていた。<br>" +
...
async function step_q10_sustain(){
  ...
  queueLinesFromBlock(
    ...
    "記録係は奥の椅子に座り、{{player}}に手前の椅子に座るように手で促す。<br>" +
  ...
  queueLinesFromBlock(
    ...
    "{{player}}が入ってきた扉からこの部屋を出ようとした時、白いカーテンの向こうに誰かが入ってきた気配を感じる。<br>" +
...
async function step_exit(){
  ...
  queueLinesFromBlock(
    ...
    "{{player}}の数メートル先で、記録係は音声を発した。<br>" 
  );

  let line="";
  if(STATE.timeline==="current" && STATE.valence!=="neg"){
    line = "記録係：今までの回答を保存し、出力できるように準備中です。<br>記録係：{{player}}と《"+STATE.target+"》の関係は、なお継続中と認識されました。<br>記録係：継続的な関係は、当機関にとって今後も重要な観測対象です。<br>記録係：今回の回答作業が、{{player}}にとって再認識の機会となっていましたら幸いです。";
  }else if(STATE.timeline==="past" && STATE.valence!=="neg"){
    line = "記録係：今までの回答を保存し、出力できるように準備中です。<br>記録係：{{player}}と《"+STATE.target+"》の関係は、既に終了したと認識されました。<br>記録係：しかし残された記録は、貴重な標本です。<br>記録係：当機関にとっても。<br>記録係：おそらくは{{player}}にとっても。";
  }else if(STATE.timeline==="current" && STATE.valence==="neg"){
    line = "記録係：今までの回答を保存し、出力できるように準備中です。<br>記録係：{{player}}と《"+STATE.target+"》の関係は、負の感情を帯びながらも継続中であると認識されました。<br>記録係：それが効率的ではない関係だとしても、故にこそ認められる特異性もあるでしょう。<br>記録係：今回の回答作業が、{{player}}にとって再認識の機会となっていましたら幸いです。";
  }else{
    line = "記録係：今までの回答を保存し、出力できるように準備中です。<br>記録係：{{player}}と《"+STATE.target+"》の関係は、負の感情を残したまま終了したと認識されました。<br>記録係：個々人により異なりますが、断絶を感じて痛みを覚える方もいるでしょう。<br>記録係：複雑な感情を伴う関係性は、言語で整理することにより客観視出来る場合もあります。<br>記録係：今回の回答作業が、{{player}}にとって良い機会となっていましたら幸いです。";
  }
  ...
  queueLinesFromBlock(
    ...
    "ゲートの傍らで、記録係は足を止めて{{player}}へとゆっくりと振り返る。<br>" +
