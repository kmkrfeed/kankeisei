...
async function step_intro(){
  ...
  queueLinesFromBlock(
    ...
    "記録係：考えがまとまらない時は、あなたの発言として心ゆくまで話していただいても構いません。<br>" +
...
STATE.timeline = (res.value.startsWith("past_")) ? "past" : "current";
  ...
  queueLinesFromBlock(
    (STATE.timeline==="current")
      ...
      : "記録係：展示に回答を記録いたします。<br>記録係：{{player}}様と《${STATE.target}》の関係は過去に属するものであると認識しました。<br>記録係：以降の問い掛けの一部は過去形に変更し、質問・記録します。<br>記録係：それでは、次の部屋に移動します。<br>記録係：私が誘導しますので順路に従ってお進みください。<br>壁面を見ると、先程までとは違い、セピア色の古い写真が流れている。<br>{{player}}はさらにその中の一つに他人ではなく、自分と《${STATE.target}》が写っているものを見つけた。<br>次の部屋へと先導する記録係の後ろで、{{player}}は歩みを進めるのだった。<br>しばらく歩いたのち、記録係が声を発する。<br>記録係：……ご連絡です。第二の部屋に到着いたします。"
  );
  await waitQueueEmpty();
...
async function step_q2_valence(){
  ...
  queueLinesFromBlock(
    ...
    "筒状の機械は記録係の背丈の半分ほどの高さで、不規則に点在している。<br>" +
...
async function step_q3_heart_share(){
  ...
  queueLinesFromBlock(
    ...
    "記録係：また、《${STATE.target}》のことを考える時、あなたはどんな気持ちをどの程度抱くでしょうか。<br>" +
    "記録係：柔らかな気持ち。溢れるほどの気持ち。<br>" +
    "記録係：刺々しい気持ち。押さえ込んでいる気持ち。<br>" +
  ...
  const free = await askFree(
    "記録係：また、《${STATE.target}》のことを考えている時のあなたの気持ちとその程度を言葉で表現してください。",
    "感じていること・程度など（自由回答）"
  );
...
async function step_q8_respect(){
  ...
  queueLinesFromBlock(
    "記録係：展示に回答を記録いたします。<br>" +
    "{{player}}が壁を見ると、創設者の肖像画の右隣に、新たな肖像画が増えていた。<br>" +
...
async function step_q10_sustain(){
  ...
  queueLinesFromBlock(
    ...
    "記録係は奥の椅子に先に座り、{{player}}へと手前の椅子に座るように手で促してくる。<br>" +
    "空間の奥に視線を向けると、白いカーテンで遮られている。<br>" +
    "その先に何があるのかは見えない。<br>" +
    "上を見れば、ガラス張りの天井から丸い月が覗いていた。<br>" +
    ...
    "記録係：反対に、《${STATE.target}》との関係性が過去である場合には、関係がなぜ続かなくなったのか、お尋ねします。<br>" +
    "記録係：離れることとなった契機・経緯、そして関係性の変化の本質を、もし許されるのならば保存いたします。<br>" +
    "記録係：回答内容は自由に、あなたの主観で構いません。<br>" +
    "記録係：《${STATE.target}》と{{player}}様、あなた自身を見つめ直して、言葉を紡いでください。<br>" +
    "記録係：それでは、あなたの実情に沿ったものを回答してください。"
  ...
  if(STATE.timeline==="past"){
    ...
    if(STATE.valence==="neg"){
      ...
      const why = await askFree("記録係：続けて質問します。あなたはなぜそう思いますか？", "理由・状況・希望・懸念など（自由回答）");
      setAnswer("存続の診察室","存続の診察室", q.replace(/^【[^】]+】<br>記録係：/,""), res.label, why||null);
    }
  }
...
async function step_exit(){
  ...
  queueLinesFromBlock(
    ...
    "月は何も変わらず、物言わず、わずかな光を真っ暗な空からただ注いでいた。<br>" +
